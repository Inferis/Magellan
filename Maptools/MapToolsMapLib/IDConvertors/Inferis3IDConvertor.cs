...
namespace MapToolsLib
{
	...
	/// </summary>
	public class InferisEU3CompatIDConvertor : IIDConvertor
	{
        ushort seastart;
        public InferisEU3CompatIDConvertor(ushort seazoneStartIndex) {
            seastart = seazoneStartIndex;
        }
		...
		public int ConvertID32( ushort id ) {
            ...
            if (id >= seastart)
                result = ((result & 0xF00) << 8) | ((result & 0xF0) << 4) | (0xFF - ((result & 0xF) << 1));
            else
